...
- `pixi run prepare-kimera-mesh-vicon-room-1-coverage-adaptive` builds the seed-only Kimera-only coverage-aware adaptive mesh triangle dataset; `pixi run prepare-kimera-mesh-vicon-room-1-coverage-adaptive-image-fallback` adds the Kimera-only image-plane fallback for uncovered regions. The `*-sfm-fallback` variants are ablation-only and should not be treated as the final algorithm.
